...
if [ "$BATTERY_CAPACITY" -le "$LOW_THRESHOLD" ] && [ "$BATTERY_CAPACITY" -gt "$CRITICAL_THRESHOLD" ] && [ "$BATTERY_STATUS" = "Discharging" ]; then
...
if [ "$BATTERY_CAPACITY" -le "$CRITICAL_THRESHOLD" ] && [ "$BATTERY_CAPACITY" -gt "$EMERGENCY_THRESHOLD" ] && [ "$BATTERY_STATUS" = "Discharging" ]; then
...
if [ "$BATTERY_CAPACITY" -le "$EMERGENCY_THRESHOLD" ] && [ "$BATTERY_STATUS" = "Discharging" ]; then
